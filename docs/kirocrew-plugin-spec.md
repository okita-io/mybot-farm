...
> **Finding (2026-09-26, verified):** probed the backend binary. `kirocrew agent {list,create,update,delete}` manages **Crew Member bindings only** — `create` takes `--name`/`--kiro-agent`/`--workspace`/`--memory-store`, NOT template content. There is **no full-agent import verb**. So the plugin **writes the template JSON directly** to `~/.kiro/agents/<name>.json`, then binds a member via `kirocrew agent create --kiro-agent <name>`. A member = `NAME → KIRO_AGENT template + WORKSPACE + MEMORY_STORE` (confirmed by `agent list`). This makes the plugin the owner of template correctness (schema, reserved-name avoidance, the D3 allow-list). Full detail in **D1**.
...
- **D2 — skills placement: RESOLVED — steering files.** Plant each GAF `skills[].content` as a steering `.md` under a namespaced dir (e.g. `.kiro/steering/farm/<slug>/<skill-name>.md`) and point the template's `resources[]` at it (`file://.kiro/steering/farm/<slug>/**/*.md`), matching how shipped templates already load steering (`resources: ["file://.kiro/steering/**/*.md"]`). Rationale: (a) faithful to the native model — KiroCrew skills *are* steering files, so this round-trips cleanly on export; (b) keeps the `prompt` readable instead of ballooning it with every skill body; (c) namespacing under `farm/<slug>/` makes `farm_reinstall --clean` and uninstall a clean directory removal without touching the user's own steering. Fallback: if a skill has no `content` (name/description only), inline a one-line reference into the prompt rather than writing an empty file.
- **D3 — default tool allow-list: RESOLVED — deny-by-default, read/search/web only.** A planted third-party agent's template gets a conservative `tools`/`allowedTools`: read/search/web/introspect and the safe read-only `@kirocrew-*` verbs — explicitly **NOT** `execute_bash`/`fs_write`/destructive MCP verbs, regardless of what the pack's `plugins[]` request. The plant output tells the user exactly which capabilities were withheld and how to grant them (edit the template, or a future `farm_plant --grant <tool>`). Rationale: install is a trust boundary; a stranger's agent must not arrive with shell/write access on the user's machine. This mirrors the farm principle "scrub or don't ship" applied to *capabilities*, not just secrets. This is a safety default and is non-negotiable for v1.
- **D4 — routines auto-scheduling: RESOLVED — default OFF.** GAF `routines[]` are recorded in the planted agent's prompt as documented routines; the plugin does **not** create `kirocrew-cron` jobs on install. A separate explicit step (`farm_plant --schedule-routines`, or the user asking the agent to schedule them) turns them into cron. Rationale: silently scheduling a stranger's recurring jobs on the user's machine at install time is a footgun (unexpected background execution, resource use, outbound calls). Opt-in only.
