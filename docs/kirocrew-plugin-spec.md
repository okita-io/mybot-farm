...
- **D1 — import path:** direct file-write to `~/.kiro/agents/` vs a supported `kirocrew` CLI/RPC import verb. *Verify first* — determines whether we risk a Hermes-"GAP 2"-style bug. Probe the backend binary's help output in a work session.
