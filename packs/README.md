...
The landing page serves the same files from `web/public/packs/` so people can download a copy, then in Grok Bot: **New → Create new agent → Edit Profile**. Adding a Bot copies configuration only — not the author’s computer, logins, or chat history ([Grok Bot docs](https://docs.x.ai/grok-bot/bots)). Keep `packs/` and `web/public/packs/` in sync.
